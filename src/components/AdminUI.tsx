...
import { savePredefinedItem, getPredefinedItems } from '../store';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Start Time (Optional, 24hr e.g. 09:30)</label>
              <input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
            </div>
            <div>
            ...
            {items.map(item => (
              <div key={item.id} className="p-3 border border-gray-200 rounded-md bg-gray-50">
                ...
                <div className="text-sm text-gray-500 mt-1">
                  {item.startTime ? `${item.startTime} • ` : ''}{item.duration} min • {item.type} • {item.location}
                </div>
              </div>
            ))}
          </div>
